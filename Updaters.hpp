...
void ProcessSchedulingCalculator::updateProcessTable(const short int &newProcessCount){
...
void ProcessSchedulingCalculator::showAlertMessage(const std::string &message, const bool &messageTypeIsSuccess){
...
bool ProcessSchedulingCalculator::validateInputs(){
...
std::string ProcessSchedulingCalculator::getProcessColor(const short int processId){
    return  processColors_[(processId - 1) % processColors_.size()];
}


// create & update the statistics values with the result
void ProcessSchedulingCalculator::updateStatistics(const std::vector<Process> &processes){
...
std::string ProcessSchedulingCalculator::generateGanttHTML(const std::vector<GanttSegment> &segments){
...
void ProcessSchedulingCalculator::updateGanttChart(const std::vector<GanttSegment> &segments){
    if (segments.empty()){
...
void ProcessSchedulingCalculator::clearResults(){
